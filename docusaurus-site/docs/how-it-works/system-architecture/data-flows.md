...
**Flow B:** Alice adds her first credential by (1) generating an idOS keypair for her, which she backs up using [derived keys](../encryption.md#derived-key-generation). The dashboard then (2) sends Alice the idOS public key. (3) Fractal ID then adds Alice's credential to the idOS.  

![User joins idOS and gets credential](/assets/idOS_Gitbook_user-flow-1.png)
...
![User manages data on dashboard](/assets/idOS_Gitbook_user-flow-2.png)
...
![User grants data access](/assets/idOS_Gitbook_user-flow-3.png)
...
![User grants dApp data access](/assets/idOS_Gitbook_user-flow-4.png)
